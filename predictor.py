...
def get_stats_for_fighter(df, fighter, inactive_days):
    ...
    decay_factor = 1 - DECAY_RATE * max(
        0, (inactive_days - DECAY_THRESHOLD) / 365
    ) ** 2
...
def build_input_df(fighter1, fighter2):
    ...
    # --- stats ---
    last_date_f1 = pd.to_datetime(
        df_stats[df_stats["fighter1"] == fighter1].iloc[-1]["date"]
    )
    last_date_f2 = pd.to_datetime(
        df_stats[df_stats["fighter1"] == fighter2].iloc[-1]["date"]
    )
    ...
    diffs = {
        f"diff_{stat}": stats_f1.get(stat, 0) - stats_f2.get(stat, 0)
        for stat in stats
    }
    ...
    input_df = pd.DataFrame([[0.0] * len(selected_features)],
