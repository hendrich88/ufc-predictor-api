...
def predict_fight_with_shap(fighter1: str, fighter2: str) -> dict:
    """
    Predikce zápasu včetně SHAP hodnot seskupených podle kategorií.
    Řeší problém s 3D shap hodnotami.
    """
    # Vytvoření vstupních dat pro oba směry
    input_1 = build_input_df(fighter1, fighter2)
    input_2 = build_input_df(fighter2, fighter1)
    ...
    # pokud je list (binární klasifikace), vezmeme třídu 1
    if isinstance(shap_values_all, list) and len(shap_values_all) == 2:
        shap_values_f1 = shap_values_all[1]
    else:
        shap_values_f1 = shap_values_all

    # Pokud shap_values_f1 má víc než 2 dimenze, reshape na 2D
    if len(shap_values_f1.shape) > 2:
        shap_values_f1 = shap_values_f1.reshape(shap_values_f1.shape[0], -1)

    # Převod na DataFrame
    shap_df = pd.DataFrame(shap_values_f1, columns=selected_features)

    # Shrnutí SHAP hodnot podle kategorií
    shap_groups = {}
